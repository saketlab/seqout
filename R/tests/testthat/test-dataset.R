test_that("dataset accepts any accession and records what it is", {
  ...
  expect_equal(dataset(con, "SRR13927092")$kind, "run")
  expect_equal(dataset(con, "SRX10306523")$kind, "experiment")
  expect_equal(dataset(con, "SAMEA7015536")$kind, "biosample")
})

test_that("dataset rejects an accession it does not recognise", {
  expect_error(dataset(fake_con(), "nonsense"), "not an accession")
})

test_that("dataset trims surrounding whitespace", {
  expect_equal(dataset(fake_con(), "  GSE168652 ")$accession, "GSE168652")
})

test_that("dataset requires a connection", {
  expect_error(dataset("not a connection", "GSE168652"), "seqout_connection")
})

test_that("a study or series is its own project, with no request", {
  expect_equal(dataset(fake_con(), "GSE168652")$project, "GSE168652")
  expect_equal(dataset(fake_con(), "SRP310139")$project, "SRP310139")
  expect_equal(dataset(fake_con(), "CRA002740")$project, "CRA002740")
})

test_that("a study is its own sra, and a series its own geo", {
  expect_equal(dataset(fake_con(), "SRP310139")$sra, "SRP310139")
  expect_equal(dataset(fake_con(), "GSE168652")$geo, "GSE168652")
})

test_that("fields are cached, so a second read makes no new request", {
  d <- dataset(fake_con(), "GSE168652")
...
test_that("detail is NULL for a study or series", {
  expect_null(dataset(fake_con(), "GSE168652")$detail)
  expect_null(dataset(fake_con(), "SRP310139")$detail)
})
...
test_that("both spellings build the same object", {
  expect_equal(Dataset(fake_con(), "GSE168652")$kind, dataset(fake_con(), "GSE168652")$kind)
})
