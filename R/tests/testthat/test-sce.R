...
test_that("anything that is neither a matrix nor one GSM is rejected", {
  skip_if_not_installed("SingleCellExperiment")

  expect_error(seqout_sce("GSE297547"), "not a GSM accession")
  expect_error(seqout_sce(c("GSM1", "GSM2")), "one GSM accession")
})
