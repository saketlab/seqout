...
`search` finds studies across archives using the [seqout.org](https://seqout.org/) search engine. See [How search works](https://seqout.org/howsearchworks) for query semantics.
...
You can query multiple terms in one call. Terms absent from the ontology graph are reported as missing:
...
| `--no-children` | Skips child node queries for identifier-only lookups. |
...
The command lists alignment files and their access requirements before downloading:
...
The `show`, `download`, `pmid`, `author`, and `convert` commands support the `--parquet` flag. When specified, the command executes queries offline using your Parquet database source with no REST API request:
