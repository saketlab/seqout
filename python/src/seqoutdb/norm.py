"""`seqoutdb --norm`: build per-sample prompts from a project's metadata and run
them through a local GGUF model (default: saketlab/seqoutlm-1B-GGUF) served by
...
SYS_PROMPT = (
    ...
    "genetic_modification, assay, assay_category, sample_type. You only output valid JSON."
)
...
def _add_attr(attrs: dict, key, value) -> None:
...
def _study_dict(title=None, summary=None, overall_design=None) -> dict:
    out = {}
    for key, val in (
        ("title", title),
        ("summary", summary),
...
def _study_text_geo(meta: ProjectMetadataResult, sq: Seqout) -> dict:
...
def _geo_sample_attrs(sample) -> dict:
...
def _sra_sample_attrs(sample) -> dict:
...
def build_records(sq: Seqout, accession: str, on_progress=None) -> list[SampleRecord]:
    ...
    if up.startswith("GSE"):
        ...
        for s in samples:
            ...
            records.append(
                SampleRecord(s.accession, s.title or "", details, attrs)
            )
        return records
    ...
    if up.startswith(("SRP", "ERP", "DRP")):
        ...
        sample_ids = list(
            dict.fromkeys(sid for e in experiments for sid in e.samples)
        )
        records = []
    ...
    if up.startswith(("SRS", "ERS", "DRS", "SRX", "ERX", "DRX")):
        ...
        attrs = _sra_sample_attrs(detail.sample)
        details = dict(study)
        if attrs:
            details["attributes"] = attrs
        return [SampleRecord(detail.sample.accession, detail.sample.title or "", details, attrs)]
    ...
    if up.startswith("GSM"):
        ...
        attrs = _geo_sample_attrs(detail.sample)
        details = dict(study)
        if attrs:
            details["attributes"] = attrs
        return [
            SampleRecord(detail.sample.accession, detail.sample.title or "", details, attrs)
        ]
...
def hf_repo_is_private(repo: str) -> bool:
    """True if the HF repo requires auth (gated/private) and we have no access."""
    repo = repo.split(":")[0]  # drop any quant tag, e.g. ':BF16'
    headers = {}
...
class OllamaEngine:
    ...
    def ensure_ready(self, status=None) -> None:
        ...
        if shutil.which("ollama") is None:
            raise EngineError(
                ...
                "  Install it from https://ollama.com/download (or `brew install ollama`),\n"
    ...
    def _pull(self, status=None) -> None:
        if status:
